...
class VNBatchNorm(nn.Module):
    ...
    def forward(self, x):
        ...
        # Batchnorm often requires float32 weights on CUDA even for bf16/f16 inputs.
        if self.bn.weight is not None and self.bn.weight.dtype != torch.float32:
            self.bn.float()
            
        norm_bn = self.bn(norm.to(dtype=torch.float32)).to(dtype=x.dtype)
...
class VNStdFeature(nn.Module):
    ...
    def forward(self, x):
        ...
        '''
        z0 = x
        z0 = self.vn1(z0) # torch.Size([4, 512, 3])
        z0 = self.vn2(z0) # torch.Size([4, 256, 3])
        ...
        return x_std, z0 # torch.Size([4, 1024, 3]), torch.Size([4, 3, 3])
