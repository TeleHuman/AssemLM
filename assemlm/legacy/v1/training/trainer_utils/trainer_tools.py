    ...
    def load_pretrained_backbones(model, checkpoint_path=None, reload_modules=None):
        ...
        if reload_modules:  # partial load
            ...
            for path in module_paths:
                reload_modules_list = path.split(".")
                module = model
                try:
                    for module_name in reload_modules_list:  # find the module to modify level by level
                        module = getattr(module, module_name)
                    prefix = path + "."
                    sub_state_dict = {k[len(prefix) :]: v for k, v in checkpoint.items() if k.startswith(prefix)}
                    
                    if not sub_state_dict:
                        module_param_names = set(module.state_dict().keys())
                        checkpoint_param_names = set(checkpoint.keys())
                        
                        if module_param_names.intersection(checkpoint_param_names):
                            sub_state_dict = checkpoint
                    
                    if sub_state_dict:
                        missing_keys, unexpected_keys = module.load_state_dict(sub_state_dict, strict=False)
                        if dist.get_rank() == 0:
                        ...
                        print(f"parameters not found in checkpoint '{path}'")
                except AttributeError:
