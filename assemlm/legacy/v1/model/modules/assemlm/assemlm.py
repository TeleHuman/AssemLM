...
from assemlm.legacy.v1.model.modules.point_projector import PointCloudProjector
from assemlm.legacy.v1.model.modules.point_encoder import get_point_encoder
...
class _AssemLM_Interface(nn.Module):
        ...
        if pe_type:
            ...
            try:
                try:
                    ...
                    pkg = importlib.import_module("assemlm.model.modules.pvlm")
    ...
    def build_assemlm_inputs(self, images, point_clouds, instructions, solutions=None, **kwargs):
        ...
        from assemlm.utils.point_clouds import resample_point_cloud_sequence
        ...
        for imgs, pcs, instruction in zip(images, point_clouds, instructions):
            ...
            messages.append(msg)
            point_clouds = resample_point_cloud_sequence(pcs)  # uniform point count before stacking
    ...
    def _build_inputs_embeds(
        ...
        disable_deepstack = os.environ.get("ASSEMLM_DISABLE_DEEPSTACK_VISUAL_EMBEDS", "0") in (
            "1",
            "true",
            "True",
            "yes",
    ...
    def check_pointencoder_property(self, gt_data, pred_data):
        ...
        with torch.no_grad():
            ...
            pcs_R = torch.matmul(gt_data['src_pc'].permute(0, 2, 1), R).permute(0, 2, 1) # Rotate the original source point cloud to obtain pcs_R
